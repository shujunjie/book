        ...
        messages: [
          {
            ...
            content: [
              ...
              { 
                  type: "text", 
                  // 🔥 核心修改：明确告诉 AI 只能选哪些词
                  text: `分析账单图片。提取：amount(金额数字), merchant(商户名), category(必须严格从以下列表中选择一个最匹配的: [${STANDARD_CATEGORIES.join(', ')}]), date(yyyy-MM-dd HH:mm:ss,如果图片中完全没有日期，请直接返回当前的北京时间), note(简短备注)。返回纯JSON。` 
              }
            ]
          }
        ],
